import type { InvoiceData, TemplateId, LineItem } from "../../types";
...
function ItemsTable({ items, theme }: { items: LineItem[]; theme: { primaryLight: string; primaryDark: string; border: string } }) {
    ...
    <table className="w-full text-sm border-collapse">
      <thead>
        <tr style={{ background: theme.primaryLight, color: theme.primaryDark }}>
          ...
          {hasDuration && <th className="text-left py-2 px-3 font-semibold">Durasi</th>}
          <th className="text-right py-2 px-3 font-semibold rounded-r-lg">Jumlah</th>
        </tr>
      </thead>
      <tbody>
        {items.map((it, i) => (
          <tr key={it.id} className="border-b" style={{ borderColor: "#F1F5F9" }}>
            <td className="py-2 px-3">{it.description || `Layanan ${i + 1}`}</td>
            {hasDuration && <td className="py-2 px-3 text-slate-600">{it.duration || "-"}</td>}
...
function ClassicTemplate({ data }: { data: InvoiceData }) {
  ...
  return (
    <div className="bg-white text-slate-900 p-6 rounded-2xl">
      ...
      <div className="mt-4"><ItemsTable items={data.items} theme={theme} /></div>
      <table className="w-full text-sm mt-3 border-collapse">
        <tbody>
          <tr className="border-b" style={{ borderColor: "#F1F5F9" }}><td className="py-2 px-3 font-semibold">Total Tagihan</td><td className="py-2 px-3 text-right font-semibold">{formatCurrency(String(total))}</td></tr>
...
function ModernTemplate({ data }: { data: InvoiceData }) {
  ...
  return (
    <div className="bg-white text-slate-900 p-6 rounded-2xl">
      ...
      <div className="mb-4"><ItemsTable items={data.items} theme={theme} /></div>
      <div className="rounded-2xl p-4 space-y-2" style={{ background: theme.primaryLight }}>
        <div className="flex justify-between text-sm"><span className="text-slate-600">Total Tagihan</span><span className="font-medium">{formatCurrency(String(total))}</span></div>
...
function MinimalTemplate({ data }: { data: InvoiceData }) {
  ...
  return (
    <div className="bg-white text-slate-900 p-8 rounded-2xl">
      ...
      <div className="mb-6"><ItemsTable items={data.items} theme={theme} /></div>
      <div className="border-t pt-4" style={{ borderColor: "#F1F5F9" }}>
        <div className="flex justify-between items-baseline mb-2">
          <span className="text-sm text-slate-500">Total</span>
          <span className="text-2xl font-serif font-bold">{formatCurrency(String(total))}</span>
        </div>
